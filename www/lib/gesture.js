...
(function(){
	...
	window.H5lock = function(obj){
		...
		this.chooseType = obj.chooseType || Number(window.localStorage.getItem('chooseType'));
		this.reseting = obj.reseting ? obj.resting : false;
	...
	H5lock.prototype.initDom = function(){
		...
		var str = '<h4 id="title" class="title">绘制解锁图案</h4><a href="javascript:;" id="updatePassword">重置密码</a><canvas id="canvas" width="300" height="300"></canvas>';
		wrap.setAttribute('style','position:absolute;left:0;top:0;right:0;bottom:0');
		wrap.innerHTML=str;
	...
	H5lock.prototype.storePass = function(psw){
		...
		if(this.pswObj.step == 1){
			if(this.checkPass(this.pswObj.fpassword,psw)){
				...
				document.getElementById('title').innerHTML = '两次输入不一致，重新输入';
			...
			if(this.checkPass(this.pswObj.spassword,psw)){				
				if(this.reseting){
					document.getElementById('title').innerHTML = '绘制新的解锁图案';
					this.pswObj.step = 0;
	...
	H5lock.prototype.updatePassword = function(){
		...
		//this.pswObj = {};
		this.reset();
		document.getElementById('title').innerHTML = '绘制原始解锁图案';
		// document.getElementById('title').innerHTML = '绘制原始解锁图案';
		// this.pswObj.step = 0;
		// if(this.checkPass(this.pswObj.spassword,psw)){
		// 	document.getElementById('title').innerHTML = '绘制新的解锁图案';
		...
		// }else{
		// 	document.getElementById('title').innerHTML = '原始解锁图案错误，重新绘制';
		// }
	...
	H5lock.prototype.bindEvent = function(){
		...
		this.canvas.addEventListener('touchstart',function(e){
			e.preventDefault();
			//某些android的touchmove不宜触发 所以增加此代码

...
- (void)viewDidLoad
{
    ...
    [CDVUserAgentUtil acquireLock:^(NSInteger lockToken) {
        ...
        if (appURL) {
            NSLog(@"生产环境：此方法路径应该不执行；测试环境：执行此方法；");
...
//                NSString* html = [NSString stringWithFormat:@"<html><body> %@ </body></html>", loadErr];
                ...
                // 2 显示空白页面；
                NSURL* startURL = [NSURL URLWithString:self.startPage];
                NSDictionary *htmlDic = [self.commandDelegate loadEncryptHtml:[startURL path]];
                NSString *htmlString = [htmlDic objectForKey:@"htmlString"];
                NSURL *baseURL = [htmlDic objectForKey:@"baseURL"];
...
- (void)clear{
    [CDVUserAgentUtil releaseLock:&_userAgentLockToken];
    NSURL* startURL = [NSURL URLWithString:@"pt_init_error.html"];
    NSDictionary *htmlDic = [self.commandDelegate loadEncryptHtml:[startURL path]];
    NSString *htmlString = [htmlDic objectForKey:@"htmlString"];
    NSURL *baseURL = [htmlDic objectForKey:@"baseURL"];
    [self.webView loadHTMLString:htmlString baseURL:baseURL];
